...
export class CanvasWebGPULineRenderer implements WebGPULineRenderer {
  ...
  private segments: Segment2D[] = []
  private disposed = false

  constructor(
    canvas: HTMLCanvasElement,
    options: WebGPULineRendererOptions = {},
  ) {
  ...
  setStyle(style: {
    ...
    if (style.lineWidthPx !== undefined) this.lineWidthPx = style.lineWidthPx
    if (style.alpha !== undefined) this.alpha = style.alpha
    if (style.clearBeforeRender !== undefined) this.clearBeforeRender = style.clearBeforeRender
  }
