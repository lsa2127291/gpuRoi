...
function GetOval(from: Point2, to: Point2, radius: number, contourScale: number) {
  ...
  while (num2 < 18) {
    array[num3] = from
      .add(
        new Point2(
          radius * Math.cos(num + (num2 * 10 * Math.PI) / 180),
          radius * Math.sin(num + (num2 * 10 * Math.PI) / 180),
        ),
      )
      .toShapeValue()
    num2++
...
export const generateShapeFromBrush = (
  centerPoint: IPoint,
  lastCenterPoint: IPoint,
  radius: number,
  numPoints: number,
  imageSet: ImageSet,
) => {
  ...
  if (lastCenterPoint.x === centerPoint.x && lastCenterPoint.y === centerPoint.y) {
    ...
    const ovalData = GetOval(
      new Point2(lastCenterPoint.x, lastCenterPoint.y),
      new Point2(centerPoint.x, centerPoint.y),
      radius,
      contourScale,
    )
...
export const convertPolygonsToShape = (polygons: number[][][], imageSet: ImageSet) => {
  if (!polygons) {
...
export const convertShapeToPolygons = (shape: Shape, imageSet: ImageSet) => {
...
export const getScalineDataFromBrush = (radius: number, centerPoint: IPoint) => {
...
export const initBrush = (e: MouseEvent, imageElement: any, info: Record<string, any>) => {
  const {
    drawActiveRef,
    directionParams,
    imageInfo,
...
const handleShape = (originPolygons: number[][][], brushPoylgon: Shape, imageSet: ImageSet, drawState: number) => {
  ...
  //@ts-ignore
  const newShape: Shape = originShape[operateTypeCollect[drawState]](brushPoylgon)
  const polygons = convertShapeToPolygons(newShape, imageSet)
  return polygons
}

export const doBrush = (e: MouseEvent, imageElement: any, info: Record<string, any>) => {
  const {
    drawActiveRef,
    directionParams,
    imageInfo,
...
export const drawBrushImg = (
  ...
  if (ctx) {
    ...
    let img: HTMLImageElement | null = document.querySelector('.contour-cursor')
    if (img) {
      img.src = url
    } else {
 ...
 * 获得笔刷类型
 */
export const getBrushStatus = (polygons: number[][][], indexes: number[], drawMode: string) => {
  if (drawMode === 'erase') {
    return 0
  }
...
export const getInPolyonsIndex = (pt: IPoint, polygons: number[][][]) => {
  if (!polygons || polygons.length === 0) {
    return []
  }
