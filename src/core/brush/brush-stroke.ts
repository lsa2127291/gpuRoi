...
export function cloneVec2(p: Vec2): Vec2 {
  return { x: p.x, y: p.y }
}

export function distanceSquared2D(a: Vec2, b: Vec2): number {
  const dx = b.x - a.x
  const dy = b.y - a.y
  return dx * dx + dy * dy
}
...
function douglasPeucker(points: Vec2[], epsilonMm: number): Vec2[] {
  if (points.length <= 2) {
...
function dedupeSequential(points: Vec2[], minDistanceMm: number): Vec2[] {
  if (points.length === 0) return []

  const minDistSq = minDistanceMm * minDistanceMm
  const result: Vec2[] = [cloneVec2(points[0])]
...
function chaikinSmooth(points: Vec2[], iterations: number): Vec2[] {
  ...
  for (let iter = 0; iter < iterations; iter++) {
    const next: Vec2[] = [cloneVec2(current[0])]
...
export function simplifyStrokePoints(
  ...
  if (points.length === 0) return []
  if (points.length === 1) return [cloneVec2(points[0])]
...
export class BrushStrokeBuilder {
  private points: Vec2[] = []
  private radiusMm = 0
  private mode: BrushMode = 'add'
  private readonly options: StrokeSimplifyOptions
