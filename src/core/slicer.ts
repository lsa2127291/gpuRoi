...
/** 检查平面是否与包围盒相交 */
export function planeIntersectsBoundingBox(
  normal: Vec3,
  anchor: Vec3,
  bbox: BoundingBox,
): boolean {
...
/** 计算点到平面的有符号距离 */
function signedDistance(point: Vec3, normal: Vec3, anchor: Vec3): number {
...
export function sliceMesh(
  ...
  for (let t = 0; t < triCount; t++) {
    ...
    if (onPlane0 && onPlane1 && onPlane2) continue
    ...
    // 处理顶点恰好在平面上的情况
    if (onPlane0) addUniquePoint(intersections, v0)
    if (onPlane1) addUniquePoint(intersections, v1)
    if (onPlane2) addUniquePoint(intersections, v2)
...
/** 添加不重复的点 */
function addUniquePoint(points: Vec3[], p: Vec3): void {
