...
const segmentCanvas = document.getElementById('canvas-segment');
const diffCanvas = document.getElementById('canvas-diff');
...
const bitmapCanvas2D = bitmapCanvas.getContext('2d');
if (!bitmapCanvas2D)
    throw new Error('Failed to get bitmap canvas 2D context');
const segmentCanvas2D = segmentCanvas.getContext('2d');
if (!segmentCanvas2D)
    throw new Error('Failed to get segment canvas 2D context');
const diffCanvas2D = diffCanvas.getContext('2d');
if (!diffCanvas2D)
    throw new Error('Failed to get diff canvas 2D context');
...
function createMeshColors(count) {
    const colors = [];
...
async function runSingleBenchmark(camera, anchor) {
    if (!slicer)
        return { segments: [], initTime: 0, sliceTime: 0, timings: [] };
    const allSegments = [];
    const sliceTimings = [];
    let totalInitTime = 0;
...
async function runBatchBenchmark(camera, anchor) {
    if (!batchSlicer || !batchInited)
        return null;
    try {
        for (let w = 0; w < 2; w++) {
            const warmup = await batchSlicer.sliceToBitmap(camera.viewPlaneNormal, anchor, {
        ...
        const t1 = performance.now();
        const bitmap = await batchSlicer.sliceToBitmap(camera.viewPlaneNormal, anchor, {
            viewUp: camera.viewUp,
            width: bitmapCanvas.width,
            height: bitmapCanvas.height,
            scale: segmentRenderer.scale,
...
function renderAllSegments(segments3D, camera, anchor) {
    const basis = buildLocalBasis(camera.viewPlaneNormal, camera.viewUp);
    const segments2D = projectSegments(segments3D, anchor, basis, segmentCanvas.width, segmentCanvas.height, segmentRenderer.scale);
