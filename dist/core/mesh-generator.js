 ...
 */
export function generateRandomMesh(targetVertexCount, radius = 50, center = [0, 0, 0], seed) {
 ...
 */
export function generateMeshBatch(count, minVertices = 20000, maxVertices = 1000000, seed) {
    const rng = seed !== undefined ? createRng(seed) : Math.random;
    const meshes = [];
    for (let i = 0; i < count; i++) {
        const targetVerts = Math.round(minVertices + rng() * (maxVertices - minVertices));
