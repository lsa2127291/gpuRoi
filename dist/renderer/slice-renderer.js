...
export class SliceRenderer {
    ...
    /** 更新切割参数并触发重绘 */
    async updateSlice(camera, anchor) {
        this.camera = camera;
        this.anchor = anchor;
        await this.render();
    }
    /** 设置渲染参数 */
    async setRenderStyle(color, width, scale) {
    ...
    async render() {
        ...
        try {
            // 1. 切割
            const segments3D = await this.slicer.slice(this.camera.viewPlaneNormal, this.anchor);
            // 2. 投影
            const basis = buildLocalBasis(this.camera.viewPlaneNormal, this.camera.viewUp);
            const segments2D = projectSegments(segments3D, this.anchor, basis, this.canvasRenderer.width, this.canvasRenderer.height, this.canvasRenderer.scale);
...
//# sourceMappingURL=slice-renderer.js.map